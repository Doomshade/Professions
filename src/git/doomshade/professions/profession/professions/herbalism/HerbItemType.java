...
import git.doomshade.professions.data.cache.SpawnedCacheable;
...
public class HerbItemType extends ItemType<Herb> {
    ...
    public void onLoad() {
        ...
        for (Herb herb : Herb.HERBS.values()) {
            try {
                Collection<SpawnedCacheable> cachedHerbs = CacheUtils.readCache(herb.getId(), CACHE_FOLDER);

                boolean found = false;
                for (final SpawnedCacheable c : cachedHerbs) {
                    if (herb.getId().equals(c.getId())) {
    ...
    public void onDisable() {
        for (Herb herb : Herb.HERBS.values()) {
            Collection<SpawnedCacheable> cache = new ArrayList<>();
            for (Map.Entry<Location, HerbLocationOptions> entry : herb.getLocationOptions().entrySet()) {
                HerbLocationOptions hlo = entry.getValue();
                cache.add(new SpawnedCacheable(hlo));
            }
            try {
